...
void reverse(Node**root) {
    ...
    // initializing previous node to NULL
    Node* previous = NULL;
    // initializing current node to root node
    Node* currrent = *root;

    // loop until end of the list
    while(currrent != NULL) {

        // nxt node is set to current's pointer
        Node* nxt = currrent->next;
        // current's pointer is set previous node
        currrent->next = previous;
        // previous node is set to current node
        previous = currrent;
        // current node is set to nxt node
        currrent = nxt;
    }
    // Updating root with reversed list
    *root = previous;
}  
...
int main(void) {
    ...
    reverse(&root); // reversing the linked list
    
    display(root); // print the reversed linked list
